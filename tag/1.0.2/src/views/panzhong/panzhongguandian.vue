    ...
    <section>
        ...
        <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
            <el-form :inline="true" :model="filters">
                ...
                <div>
                    ...
                    <el-form-item>
                        <el-button type="primary"   @click="batch(1)">批量点赞</el-button>
                    </el-form-item>
                    <el-form-item>
                        <el-button type="primary"   @click="batch(2)">批量浏览</el-button>
                    </el-form-item>
                </div>
            </el-form>
        </el-col>
        ...
        <el-table  ref="multipleTable" :data="datas"  highlight-current-row v-loading="listLoading" :fit="true" @row-click="change" @selection-change="selsChange" style="width: 100%;">
         ...
         <!--   <el-table-column   prop="centor_viewpoint_id" label="ID"  min-width="70"></el-table-column>-->
            <el-table-column prop="title" label="内容" min-width="200" v-if="this.title!=''" key="title">
                <template scope="scope">
                    <span  v-if="scope.row.content!=''">{{ scope.row.title}}</span>
                    <span v-else=""> {{ scope.row.play_address}}</span>
                </template>
            </el-table-column>
            <el-table-column prop="play_address" label="内容" min-width="200" v-else="" key="play_address"></el-table-column>
        ...
        <el-dialog :title=dialogTitle :visible.sync="dialogVisible"  v-loading="Loading" :close-on-click-modal="false" v-on:close="clearEdit" width="50%">
            <el-form :model="editForm" label-width="150px" :rules="editFormRules" ref="editForm">
                ...
                <template>              
                    <el-checkbox v-model="collect"    :disabled="collectdisabled" style="margin-left:30px;">发布征集</el-checkbox>
                </template>
                ...
                <el-form-item label="同步：">
                    <template>
                        <el-checkbox v-model="sync" :disabled="syncdisabled">同步到免费</el-checkbox>
                    </template>
                </el-form-item>
        ...
        <!--批量点赞-->
        <el-dialog :title="ZsTitle" :visible.sync="zhushuiVisible" :close-on-click-modal="false">
    ...
    export default {
        ...
        data() {
            return {
                ...
                collect:false,//显示是否征集
                collectdisabled:true,//征集是否可用


                sync:false,//显示是否同步,付费内容是否可以同步到免费。
                syncdisabled:true,//同步复选框是否可选。
                desin:false, //是否是免费内容
                teacherFrom:[],
                biaoqianForm:[],
                options:[],
...
//            },
            changedata: function(index){
                ...
                console.log("ispay:"+this.biaoqianForm[index].is_pay)
                ...
                //如果付费标签，同步复选框可以启用。
                if(this.biaoqianForm[index].is_pay==1)
                {
...
//                var datetime=this.filters.datetime.getFullYear() + '-' + (this.filters.datetime.getMonth() + 1) + '-' + this.filters.datetime.getDate() + ' ' + this.filters.datetime.getHours() + ':' + this.filters.datetime.getMinutes() + ':' + this.filters.datetime.getSeconds()
                ...
                let result=await api.getPanZhongList(para);
            ...
            async handleDel(index, row) {
                this.$confirm('确认删除该记录吗?', '提示', {
                    ...
                    await api.deletepanzhong( row.centor_viewpoint_id);
                    this.listLoading = false;
                    this.$message({
                        message: '删除成功',
                        type: 'success'
            ...
            async handleEdit(index, row) {
                ...
                this.zhifu=this.editForm.is_pay;
                this.syncdisabled=true;
                this.collectdisabled=true;                
                ...
                this.$refs.editForm.validate(async (valid) => {
                    if (valid) {
                        this.$confirm('确认提交吗？', '提示', {}).then(async () => {
                            ...
                            this.editForm.collect= this.collect==true?1:0; //给增征集赋值。
                            ...
                            if(this.editOrAdd){
                                ...
                                if(this.sync==true)
                                {
                                    this.editForm.is_sync=1;
                                }
                                else
                                {
                                    this.editForm.is_sync=0;
                                }
                               ...
                               // console.log("eeee:::::"+JSON.stringify(this.editForm));
                                this.editForm.play_address=this.playadddress;
                                this.editForm.update_time=String( new Date().getTime());
                                let para = Object.assign({}, this.editForm);
            ...
            async zhushuiSubmit() {
                this.$refs.zhushuiObj.validate(async (valid) => {
                    ...
                    if (valid) {
                        let res = await api.updateLikepanzhong(this.sels);
                        if (res.success) {
                            this.$message({
                                message: "注水成功",
                                type: "success"
