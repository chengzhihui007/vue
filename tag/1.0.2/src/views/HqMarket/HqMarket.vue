...
export default {
    ...
    methods: {
            ...
            handleChangeAddTP(response, file, fileList) {
                 ...
                 this.editForm.cover = "http://app2.zslxt.com/image/"+response.message;
                  //console.log(this.editForm.cover);
            },
        ...
        //显示编辑界面
            ...
            this.dialogTitle = "编辑";
               this.editOrAdd = false;
                this.dialogVisible = true;
                let res = await api.getHqMarketDetail(id);
          ...
          async addSubmit() {
               ...
               {
                 let result = await api.updateHqMarket(this.editForm);
                   if (result == "1") {
                   this.$message({
                       message: '提交成功',
                       type: 'success'
            ...
            if (this.editOrAdd == true) {
               ...
               let result = await api.addHqMarket(this.editForm);
               if (result == "1") {
                        this.$message({
                            message: '提交成功',
                            type: 'success'
         ...
         async getHqBytitle(){
				let lists=await api.getHqBytitle(document.getElementById("title").value.trim());
				let arr = [];
        ...
        async getHq() {
              let paras = {
                ...
                let list=await api.getHqMarket(paras);
               this.datas=list;
         },
        //恢复编辑页面为默认
        clearEdit(){
