    ...
    <el-tab-pane v-for="(group, index) in groups" :key="index" :label="group.groupsName" :name="group.id">
      <!--工具条--> 
        <el-form :inline="true" style="float: right">
              <el-form-item>
                <span v-if="activeName==2" style="color:red">可针对特定星级添加营销图，若不添加则默认展示全部可见营销位！</span><el-button   type="primary"  @click="handleAdd">添加</el-button>
            </el-form-item>            
        </el-form>
        <!--列表-->
        <el-table v-if="activeName==1" :data="datas" highlight-current-row v-loading="listLoading"  style="width: 100%;">
            ...
            <el-table-column prop="name" label="营销标题" width="200" >
                <template scope="scope">
            ...
            <el-table-column prop="qxlb" label="用户权限" width="350" >
                <template scope="scope">
                    <div class="deleted-text" v-if="scope.row.deleted">{{ scope.row.link }}</div>
                    <div v-else>{{ scope.row.qxlb=='1;2;3;4;5'?'全部可见':scope.row.qxlb.replace('1','游客').replace('2','已登录').replace('3','热点决策').replace('4','价值决策').replace('5','主力决策').replace(/;/g,'、') }}</div>
                </template>            
            </el-table-column>
 
            <el-table-column prop="pubTime" label="添加时间" width="300" >
                <template scope="scope">
        ...
        <el-table v-else :data="datas" highlight-current-row v-loading="listLoading"  style="width: 100%;">
            <!--<el-table-column prop="id" label="序号"  width="100" >
                <template scope="scope">
                    <div class="deleted-text" v-if="scope.row.deleted">  {{datas.length - scope.$index}}</div>
                    <div v-else>  {{datas.length - scope.$index}}</div>
            ...
            <el-table-column prop="qxlb" label="用户权限" width="200" >
                <template scope="scope">
                    <div class="deleted-text" v-if="scope.row.deleted">{{ scope.row.link }}</div>
                    <div v-else>{{ scope.row.qxlb=='2;3;4;5'?'全部可见':scope.row.qxlb.replace('2','已登录').replace('3','热点决策').replace('4','价值决策').replace('5','主力决策').replace(/;/g,'、') }}</div>
                </template>            
            </el-table-column>
            ...
            <el-table-column label="操作" min-width="200">
                <template scope="scope">
                    <div v-if="scope.row.qxlb=='2;3;4;5'">
                    <el-button  size="small" @click="handleEdit(scope.$index, scope.row)">编辑</el-button>
   ...
   <!--编辑界面-->
    <el-dialog  :title='dialogTitle'  :visible.sync="dialogVisible" :close-on-click-modal="false">
      <el-form :model="editForm" :rules="rules" ref="editForm" label-width="100px" >
                    <el-form-item v-if="activeName==2" label="活动主题" prop="name">
                    <el-input  v-model="editForm.name" placeholder="活动主题最多不超过10个汉字"></el-input>
                </el-form-item>
              <el-form-item v-if="activeName==2"  label="营销图片" prop="image" >
             <!-- <el-upload    
                  class="upload-demo"
              ...
              <el-form-item v-if="activeName==1" label="营销标题" prop="name">
                    <el-input  v-model="editForm.name" placeholder="营销标题不超过15个汉字和特殊字符"></el-input>
                </el-form-item>
                  
              <el-form-item label="链接地址" prop="link">
                    <el-input  v-model="editForm.link" placeholder="请输入链接地址"></el-input>
                </el-form-item>   
            <el-form-item v-if="activeName==1" label="用户权限" prop="qbkj">
                <el-radio-group v-if="activeName==1" v-model="editForm.qbkj">
                  <el-radio :label="true">全部可见</el-radio>
                  <el-radio :label="false">部分可见</el-radio>
                </el-radio-group>
                <div v-if="activeName==1">
                <div v-if="!editForm.qbkj">
                <el-checkbox-group v-if="activeName==1&&!this.isShowAllError" disabled v-model="editForm.qxlbs">
                    <el-checkbox label="1">游客</el-checkbox>
                    <el-checkbox label="2">已登录</el-checkbox>
                ...
                <el-checkbox-group v-else  v-model="editForm.qxlbs">
                    <el-checkbox v-if="editForm.czqxs.indexOf('1')!=-1" disabled label="1">游客</el-checkbox>
                    <el-checkbox v-else  label="1">游客</el-checkbox>
                    <el-checkbox v-if="editForm.czqxs.indexOf('2')!=-1" disabled label="2">已登录</el-checkbox>
                    <el-checkbox v-else  label="2">已登录</el-checkbox>
                    <el-checkbox v-if="editForm.czqxs.indexOf('3')!=-1" disabled label="3">热点决策</el-checkbox>
                    <el-checkbox v-else  label="3">热点决策</el-checkbox>
                    <el-checkbox v-if="editForm.czqxs.indexOf('4')!=-1" disabled label="4">价值决策</el-checkbox>
                    <el-checkbox v-else  label="4">价值决策</el-checkbox>
            ...
            <el-form-item v-else label="用户权限" prop="qxlbs">
                <el-checkbox-group v-if="editForm.qxlb=='2;3;4;5'" disabled v-model="editForm.qxlbs">
                    <el-checkbox label="2">已登录</el-checkbox>
                    <el-checkbox label="3">热点决策</el-checkbox>
                    <el-checkbox label="4">价值决策</el-checkbox>
                    <el-checkbox label="5">主力决策</el-checkbox>
...
export default {
    data() {
        var validatePass = (rule, value, callback) => {//验证标题12个汉子，或者24个字母
            ...
            if (len>20 && this.activeName == 2) {
                callback(new Error("活动主题最多不超过10个汉字"));
            } else if (len>30  && this.activeName == 1) {
                callback(new Error("标题字数不能超过15个汉字"));
            }else{
                callback();
            }
        };
        return {
            ...
            activeName:1,
            listLoading: false,            
            groups:[{groupsName:'导航',id:1},{groupsName:'退出',id:2}],
            datas:[],
            pageNum: 1,
            pageSize: 15,
            total: 0,    
            ...
            rules: {
                qbkj:[
                  ...
                  {validator:async (rule, value, callback)=>{
                      if(!value){
                        ...
                        if(ss.join(';')=='1;2;3;4;5'){
                            let resp=await api.existQbkjOfNavigationExit({rk:this.activeName})
                    ...
                    {validator:(rule, value, callback)=> {
                        if (value == null || value == '') {
                            if (this.activeName == 1) {
                                callback(new Error("请填写营销标题"));
                            }

                            if (this.activeName == 2) {
                                callback(new Error("请填写活动主题名称"));
                            }
                        } else {
                            callback()
                        }
                ...
                link: [
                  ...
                  {validator:(rule, value, callback)=>{
                        try{
                            let sss = this.activeName;
                            if ((value == null || value == '') && this.activeName == 1) {
                                callback(new Error("请填写营销页面链接"));
                                return;
                            }
                            if (value != null && value != '') {
                                new URL(value)
    ...
    methods: {
        ...
        handleBeforeUpload(file){
            if("image/jpeg"!=file.type&&"image/jpg"!=file.type&&"image/png"!=file.type){
                this.$message.warning(`图片格式不正确请重新上传`);
                return false;
            }    
        },            
                    ...
                    this.$confirm('确认提交吗？', '提示', {}).then( async () => {
                        if(this.formAdd){
                            let params={
                                ...
                                qxlb:this.activeName==1&&this.editForm.qbkj?'1;2;3;4;5': this.editForm.qxlbs.sort().join(";"),
                                type:1,        
                                operator:this.userName
                            }
                            let resp
                            if(this.activeName==1){
                              resp=await api.insertNavigation(params)
                            }
                                 if(this.activeName==2){
                              resp=await api.insertExit(params)
                            }
                            if(!resp||!resp.state){
                                this.$message({
                            ...
                            let params={
                                ...
                                image:this.editForm.image,
                                link:this.editForm.link,
                                qxlb:this.editForm.qxlbs.sort().join(";"),
                                
                                operator:this.userName
                            }
                            let resp;
                            if(this.activeName==1){
                              resp=await api.updateNavigation(params)
                            }
                            if(this.activeName==2){
                              resp=await api.updateExit(params)
                            }
                               if(!resp||!resp.state){
                                this.$message({
        ...
        handleAdd: async function () {
            let czqxs=[]
                ...
                let resp=await api.existQxOfNavigationExit(params)
                if(!resp){
                    return;
                }
                let data=resp.data;
       ...
       async handleEdit(index,row){
            ...
            this.dialogVisible = true;
            this.dialogTitle='营销位管理'
            this.isShowAllError=row.qxlb!='1;2;3;4;5'
            
            if(this.activeName==2){
                this.isShowAllError=row.qxlb!='2;3;4;5'
            }
            this.formAdd=false
        ...
        async getList() {
                ...
                let resp=await api.findNavigationExitByPage(params)
                if(!resp){
                    return;
                }
