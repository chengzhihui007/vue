    ...
    <el-tab-pane v-for="(group, index) in groups" :key="index" :label="group.groupsName" :name="group.id">
      <!--工具条--> 
        <el-form :inline="true" style="float: right">
              <el-form-item>
                <el-button  v-if="activeName==2"  type="primary"  @click="handleAdd">添加</el-button>
            </el-form-item>            
        </el-form>
        <!--列表-->
        <el-table v-if="activeName==1"  :data="datas" highlight-current-row v-loading="listLoading"  style="width: 100%;">
            ...
            <el-table-column prop="link" label="链接" width="350" >
              ...
              <el-table-column  prop="ktqx" label="按钮状态" width="200" >
                <template scope="scope">
            ...
            <el-table-column label="操作" min-width="200">
                <template scope="scope">
                    ...
                    <el-button v-if="scope.row.fmid==1300100" type="danger" size="small"  @click="handleDel(scope.$index, scope.row,true)" >取消禁用</el-button>
                    </div>
                    <div v-else>
                    <el-button size="small" @click="handleEdit(scope.$index, scope.row)">编辑</el-button>
                    <el-button v-if="scope.row.fmid==1300100" type="danger" size="small"  @click="handleDel(scope.$index, scope.row,false)" >禁用</el-button>
                    </div>
                </template>
            </el-table-column>
        </el-table>
        <el-table v-else  :data="datas" highlight-current-row v-loading="listLoading"  style="width: 100%;">
            ...
            <el-table-column prop="pubTime" label="添加时间" width="200" >
                <template scope="scope">
                    <div class="deleted-text" v-if="scope.row.deleted">{{ scope.row.pubTime }}</div>
                    <div v-else>{{ scope.row.pubTime }}</div>
                </template>            
    ...
    <el-dialog  :title='dialogTitle'  :visible.sync="dialogVisible" :close-on-click-modal="false">
      <el-form :model="editForm" :rules="rules" ref="editForm" label-width="100px" >
            ...
            <el-form-item label="按钮状态" >
                    <el-radio-group v-model="editForm.ktqx">
...
export default {
    data() {
        ...
        return {
            ...
            activeName:1,
            listLoading: false,            
            groups:[{groupsName:'启动开屏广告',id:1},{groupsName:'首页弹框广告',id:2}],
            datas:[],
            pageNum: 1,
            pageSize: 15,
            total: 0,    
                ...
                qbkj:[
                  ...
                  {validator:async (rule, value, callback)=>{
                      if(!value){
                        ...
                        if(ss.join(';')=='2;3;4;5'){
                            let resp=await api.existQbkjOfHomeBox({type:2})
    ...
    methods: {
        ...
        async handleDel(index, row,cancel) {
            if(cancel==undefined){
                   ...
                   this.$confirm(cancel?'确认取消吗？':'若禁用该营销入口，则用户将不可见，确认要禁用该入口吗？', '提示', {
                        type: 'warning'
                    }).then(async () => {
                        if(cancel){
                                        let resp=await api.cancelDisableFreeUser(row.id)
                                     if(!resp||!resp.state){
                                    this.$message({
                                            message: '取消失败: '+(resp?resp.data:''),
                                            type: 'error'
                                        });
                                   }else if(resp.state){
                                         this.$message({
                       ...
                       let resp=await api.disableFreeUser(row.id)
                                     if(!resp||!resp.state){
                                    this.$message({
                                            message: '禁用失败: '+(resp?resp.data:''),
                                            type: 'error'
                                        });
                                   }else if(resp.state){
                                         this.$message({
                    ...
                    this.$confirm('确认提交吗？', '提示', {}).then( async () => {
                        if(this.formAdd){
                            let params={
                                ...
                                qxlb: this.editForm.qbkj?'2;3;4;5': this.editForm.qxlbs.sort().join(";"),
                                type:2,        
                                operator:this.userName
                            }
                        
                           let resp=await api.insertHomeBox(params)
                            if(!resp||!resp.state){
                                this.$message({
                            ...
                            if(this.activeName==1){
                                resp=await api.updateOpenScreen(params);
                            }else{
                                resp=await api.updateRightBottomCarousel(params)
                            }
        ...
        handleAdd: async function () {
            if(this.activeName==1&&this.datas.length>=5){
                this.$message({message: '最多仅能添加5个轮播图: '+((resp?resp.data:'')),
                              type: 'error'});  
                              return              
            }
            let czqxs=[]
                ...
                let resp=await api.existQxOfHomeBox(params)
                if(!resp){
                    return;
                }
                let data=resp.data;
...
this.isShowAllError=row.qxlb!='2;3;4;5'
        ...
        async getList() {
                ...
                let resp;
                if(this.activeName==1){
                resp=await api.findOpenScreen(params)
                }else{
                resp=await api.findHomBoxByPage(params)
                }
                if(!resp){
                    return;
                }
