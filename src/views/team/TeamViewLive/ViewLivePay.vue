<template>
    <section>
        ...
        <el-table  ref="multipleTable" :data="datas"  highlight-current-row v-loading="listLoading" :fit="true" @row-click="change" @selection-change="selsChange" style="width: 100%;">
            ...
            <el-table-column prop="bagname" label="所属锦囊" min-width="50"></el-table-column>
        ...
        <el-dialog :title=dialogTitle :visible.sync="dialogVisible"  v-loading="Loading" :close-on-click-modal="false" v-on:close="clearEdit" width="50%">
            <el-form :model="editForm" label-width="150px" :rules="editFormRules" ref="editForm">
                <el-form-item label="所属锦囊：" prop="bagname">
                    <el-select  v-model="editForm.bagname"  placeholder="请选择锦囊"  @change="changebag(editForm.bagname)">
                        <el-option
                                v-for="(item,index) in bagForm"
                                :key="index"
                                :label="item.name"
                                :value="index">
                        </el-option>
                ...
                <el-form-item label="同步至免费：" v-if="editOrAdd">
                    <el-radio-group v-model="radeo2">
                        <el-radio :label="0" >不同步</el-radio>
                        <el-radio :label="1" >同步</el-radio>
                        <el-radio :label="2" >同步标记为付费</el-radio>
                    </el-radio-group>
                </el-form-item>
                <el-form-item label="同步至免费：" v-else="">
                    <el-radio-group disabled  v-model="radeo2">
                        <el-radio :label="0" >不同步</el-radio>
                        <el-radio :label="1" >同步</el-radio>
                        <el-radio :label="2" >同步标记为付费</el-radio>
                    </el-radio-group>
...
<script>
    ...
    import {viewlivepayList,viewlivepaysave,viewlivepayupdate,viewlivepaydelect,viewlivepayupdateview,getQiyonglist,teamBagList,selectpaytwo} from "../../../api/ydApi.js";
...
//            },
            ...
            changebag: function(index){
                console.log(this.bagForm[index])
                this.bagmake=true;
                this.bagid=this.bagForm[index].bag_id;
                this.bagname=this.bagForm[index].name;
            },
...
//                var datetime=this.filters.datetime.getFullYear() + '-' + (this.filters.datetime.getMonth() + 1) + '-' + this.filters.datetime.getDate() + ' ' + this.filters.datetime.getHours() + ':' + this.filters.datetime.getMinutes() + ':' + this.filters.datetime.getSeconds()
            ...
            async handleDel(index, row) {
                ...
                this.$confirm('确认删除该记录吗?', '提示', {
                    ...
                    viewlivepaydelect(paradel).then(res => {
                        this.listLoading = false;
                        this.$message({
                            message: '删除成功',
                            type: 'success'
            ...
            async handleEdit (index, row) {
                ...
                this.radeo=this.editForm.is_end;
                this.radeo2=this.editForm.is_free;
            ...
            async editSubmit() {
                ...
                selectpaytwo(parat).then(res => {
                    console.log(res.data.list.length)
                    if(res.data.list.length>=5 && this.radeo2==2){
                        this.$confirm('每日只可标记5条付费观点', '提示', {})
                    }else{
                            ...
                            if (valid) {
                                this.$confirm('确认提交吗？', '提示', {}).then(async () => {
                                    ...
                                    if(this.editOrAdd){
                                        ...
                                        this.editForm.bag_id=this.bagid;
                                        this.editForm.bagname=this.bagname;
                                        ...
                                        let para = Object.assign({}, this.editForm);
                                        console.log(para)
                                        viewlivepayupdate(para).then(res => {
                                            this.dialogVisible = false;
                                            this.$message({
                                                message: '提交成功',
                                                type: 'success'
            ...
            async zhushuiSubmit() {
                this.$refs.zhushuiObj.validate(async (valid) => {
                    ...
                    if (valid) {
                        viewlivepayupdateview(this.sels).then(res => {
                            this.$message({
                                message: "注水成功",
                                type: "success"
                            });
