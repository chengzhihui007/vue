    ...
    <section>
            ...
            <el-form :inline="true" :model="filters">
                ...
                <el-form-item  style="margin-right: 10px;" class="inline fl">
                    <el-select  v-model="filters.child_pool_name"  clearable   placeholder="请选择查询股池">
                        <el-option key="0" label="抢筹股池" value="抢筹股池"/>
                        <el-option key="1" label="龙虎资金" value="龙虎资金"/>
                        <el-option key="2" label="高管资金" value="高管资金"/>
                        <el-option key="3" label="主力资金" value="主力资金"/>
                    </el-select>
                </el-form-item>
                <el-form-item>
                ...
                <el-form-item>
                    ...
                    <el-button  type="primary" @click="exportZiJinJieMi()" >导出</el-button>
                </el-form-item>
                </div>
            </el-form>
        </el-col>
        <el-table  ref="multipleTable" :data="datas"  highlight-current-row v-loading="listLoading" :fit="true" @selection-change="selsChange" style="width: 150%;">
            ...
            <el-table-column prop="plate_name" label="股池分类" min-width="100">
                 <template scope="scope">
                    <div class="deleted-text">{{ scope.row.stockPoolType }}</div>
                </template>
            </el-table-column>
        ...
        methods:{
            ...
            async getList(para){
                para = {
                    page : this.page,
                    size : this.size,
                    stockPoolType : this.filters.child_pool_name,
                ...
                await api.selectZiJinJieMiHistoryByPage(para).then(res => {
                    let result=res.data.result;
                    this.total = res.data.total;
                    this.datas = result
                });
            },
            async exportZiJinJieMi(para) {
                ...
                api.exporZiJinJieMiByPage(para)
