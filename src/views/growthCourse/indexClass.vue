  ...
  <section>
    ...
    <el-col :span="24" class="toolbar">
      <el-form :inline="true">
        ...
        <el-select v-model="queryForm.synchron" placeholder="是否同步指标板块">
    ...
    >
     ...
     <!--  <el-table-column prop="id" label="序号" min-width="100"></el-table-column> -->
     	...
     	<el-table-column prop="" label="序号" min-width="100" align="center">
				  <template scope="num">
                     <div>{{indexList.length - num.$index}}</div>
                </template>
			</el-table-column>
      ...
      <el-table-column prop="synchron" label="是否同步指标板块" min-width="140" align="center">
          <template scope="scope">
...
export default {
  ...
  methods: {
    ...
    this.indexFrom.forEach(element => {

      if(element.id == id){
        if(element.jurisdictionValue== 0){
           this.strategyBlockListByKey(0);
        } else if (element.jurisdictionValue == -2) {
            this.strategyBlockListByKey(-2);
        } else if(element.jurisdictionValue == 3){
          this.strategyBlockListByKey(3);
        }else if(element.jurisdictionValue == 4){
          this.strategyBlockListByKey(4);
        }else if(element.jurisdictionValue == 5){
          this.strategyBlockListByKey(5);
        }
      }
    });
    },
          ...
          this.$refs["addForm"].validate(async valid => {
            ...
            if (valid) {
             ...
             /*this.indexFrom.forEach(element => {
              ...
              let res = await api.addIndexStrategy(this.addForm);
              if (res.state == true) {
                this.$message({
                  message: "添加成功",
                  type: "success"
      ...
      //this.addForm.cover_url = imageDefault + res.message;
    ...
    fnUploadRequest(option) {
        let extensionName = "." + option.file.name.substr(option.file.name.lastIndexOf(".") + 1);
        utils.ossput(new Date().getTime().toString() + extensionName, option.file).then(res => {
      ...
      this.$refs.addForm.validate(async valid => {
        if (valid) {
          ...
          this.indexFrom.forEach(element => {
            ...
            this.addForm.account = JSON.parse(sessionStorage.getItem('user')).name;
          let res = await api.editIndexClass(this.addForm);
          if (res.state == true) {
            this.$message({
              message: "修改成功",
              type: "success"
     ...
     if(row.synchron=="是"){
      ...
      this.indexFrom.forEach(element => {
        if(element.title == row.setsystem){
          if(element.jurisdictionValue== 0){
              this.strategyEditBlockListByKey(0,row)
          } else if (element.jurisdictionValue == -2) {
             ...
             this.strategyEditBlockListByKey(5,row)
          }
        }
      });
      //});
    ...
    async handleDel(id) {
      this.$confirm("若删除该视频不可恢复,确定删除该视频吗?", "提示", {
        ...
        let res = await api.deleteIndexStrategy(id);
        if (res.state == true) {
          this.$message({
            message: "删除成功！",
            type: "success"
    ...
    let res =  await api.indexClassList(this.query);
    this.indexList=res.data.result;
    this.total = res.data.total
        this.currentPage = res.data.page;
    },
     async selectIndexList() {
      let res = await api.findIndexManageListAll();
      this.indexFrom = res.data;
    },
    ...
    async zhushuiSubmit() {
      this.$refs.zhushuiObj.validate(async valid => {
        ...
        if (valid) {
          ...
          let res = await api.indexClassZhuShui(this.sels);
          if (res.state == true) {
            this.$message({
              message: "注水成功",
              type: "success"
   ...
   async strategyBlockList(){
       var res =await  api.strategyBlockList(2);//策略板块
       this.blockList = res.data
    },
    async strategyBlockListByKey(key){
            await  api.strategyBlockList(2).then((res)=>{
                this.blockList = res.data
                ...
                this.$set(eles , 'dis',0)
                 ...
                 api.strategyBlockListByKey(key,2).then((res)=>{
