...
<br/>
        ...
        <el-table :data="datas" highlight-current-row v-loading="listLoading"  style="width: 100%;">
            ...
            <el-table-column prop="name" label="营销入口位" width="200" >
                <template scope="scope">
            ...
            <el-table-column prop="name" label="活动主题" width="200" >
                <template scope="scope">
                    <div class="deleted-text" v-if="scope.row.deleted">{{ scope.row.themeName }}</div>
                    <div v-else>{{ scope.row.themeName }}</div>
                </template>
            </el-table-column>
            ...
            <el-table-column prop="link" label="链接" width="300" >
                <template scope="scope">
            ...
            <el-table-column label="操作" min-width="200">
                <template scope="scope">
                    ...
                    <el-button v-if="scope.row.fmid==1170600" type="danger" size="small"  @click="handleDel(scope.$index, scope.row,true)" >取消禁用</el-button>
                    </div>
                    <div v-else>
                    <el-button size="small" @click="handleEdit(scope.$index, scope.row)">编辑</el-button>
                    <el-button v-if="scope.row.fmid==1170600" type="danger" size="small"  @click="handleDel(scope.$index, scope.row)" >禁用</el-button>
                    </div>
                </template>
            </el-table-column>
        </el-table>
    ...
    <el-dialog  :title='dialogTitle'  :visible.sync="dialogVisible" :close-on-click-modal="false">
      <el-form :model="editForm" :rules="rules" ref="editForm" label-width="100px" >
              ...
              <el-form-item  label="活动主题" prop="themeName">
                  <el-input  v-model="editForm.themeName" placeholder="活动主题最多不超过10个汉字"></el-input>
              </el-form-item>
...
export default {
        ...
        return {
            ...
            rules: {
                name: [
                    ...
                    {max: 10, message: '服务名称最多不超过10个汉字', trigger: 'blur'}
                ],
                 themeName: [
                ...
                link: [
                  ...
                  {validator:(rule, value, callback)=>{
                      ...
                      debugger;
                        try{
                            if ((value != null && value != '') || this.editForm.id == 4) {
                                if (value == null || value == '') {
                ...
                image: [
                    {required: true, message: '请上传服务背景图片', trigger: 'blur'}
                ],              
                replyContent: [
    ...
    methods: {
                ...
                if (valid) {
                    this.$confirm('确认提交吗？', '提示', {}).then( async () => {
                        if(this.formAdd){
                            ...
                            let resp=await api.insertProductService(params)
                               if(!resp||!resp.state){
                                this.$message({
                            ...
                            let params={
                                ...
                                image:this.editForm.image,
                                link:this.editForm.link,
                                qxlb:this.editForm.qxlb,
                                themeName:this.editForm.themeName,
                                operator:this.userName
                            }
                            let resp=await api.updateFreeUser(params)
                               if(!resp||!resp.state){
                                this.$message({
        ...
        async getList() {
                ...
                let resp=await api.findFreeUserByPage(params)
                if(!resp){
                    return;
                }
