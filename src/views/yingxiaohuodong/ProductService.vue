...
<br/>
  <!--工具条-->
        <el-form :inline="true">
           <el-form-item>
                <el-date-picker v-model="filters.date" type="date" placeholder="创建时间查询">
                </el-date-picker>
            </el-form-item>
            <el-form-item > 
        ...
        <el-table :data="datas" highlight-current-row v-loading="listLoading"  style="width: 100%;">
            ...
            <el-table-column prop="link" label="PC详情链接" width="300" >
                <template scope="scope">
                    <div class="deleted-text" v-if="scope.row.deleted">{{ scope.row.link }}</div>
                    <div v-else>{{ scope.row.link }}</div>
                </template>            
            </el-table-column>
                        <el-table-column prop="link2" label="APP详情链接" width="300" >
                <template scope="scope">
    ...
    <el-dialog  :title='dialogTitle'  :visible.sync="dialogVisible" :close-on-click-modal="false">
      <el-form :model="editForm" :rules="rules" ref="editForm" label-width="100px" >
              <el-form-item label="服务名称" prop="name">
                    <el-input  v-model="editForm.name" placeholder="服务名称不超过10个汉字"></el-input>
                </el-form-item>
             ...
             <!-- <el-upload    
              ...
              <el-form-item label="PC链接地址" prop="link">
                    <el-input  v-model="editForm.link" placeholder="请输入服务跳转地址"></el-input>
                </el-form-item>            
                <el-form-item label="APP链接地址" prop="link2">
                    <el-input  v-model="editForm.link2" placeholder="请输入服务跳转地址"></el-input>
...
export default {
    data() {
        var validatePass = (rule, value, callback) => {//验证标题12个汉子，或者24个字母
            ...
            if(len>20){
                callback(new Error("服务名称最多不能超过10个汉字"));
            }else{
                callback();
            }
        };
    ...
    methods: {
                ...
                if (valid) {
                    this.$confirm('确认提交吗？', '提示', {}).then( async () => {
                        if(this.formAdd){
                            ...
                            let resp=await api.updateProductService(params)
                               if(!resp||!resp.state){
                                this.$message({
        ...
        async getList() {
                ...
                let resp=await api.findProductServiceByPage(params)
                if(!resp){
                    return;
                }
