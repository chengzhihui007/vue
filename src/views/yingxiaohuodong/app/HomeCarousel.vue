    ...
    <el-tab-pane v-for="(group, index) in groups" :key="index" :label="group.groupsName" :name="group.id">
      <!--工具条-->
        ...
        <el-table  :data="datas" highlight-current-row v-loading="listLoading"  style="width: 100%;">
            ...
            <el-table-column prop="qxlb" label="用户权限" width="100" >
                <template v-if="activeName==1" scope="scope">
                 未付费
                </template>
                <template v-else scope="scope">
                    <div class="deleted-text" v-if="scope.row.deleted">{{ scope.row.link }}</div>
                    <div v-else>{{ scope.row.qxlb=='3;4;5'?'全部可见':scope.row.qxlb.replace('3','多空决策').replace('4','风口决策').replace('5','主力决策').replace(/;/g,'、') }}</div>
                </template>
            </el-table-column>
      ...
      <el-form :model="editForm" :rules="rules" ref="editForm" label-width="100px" >
            ...
            <el-form-item v-if="activeName==2" label="用户权限" prop="qbkj">
                ...
                <el-checkbox-group  v-model="editForm.qxlbs">
                    <el-checkbox v-if="ffqx3.length<5||editForm.czqxs.indexOf('3')!=-1"  label="3">多空决策</el-checkbox>
                    <el-checkbox v-else disabled label="3">多空决策</el-checkbox>
                    <el-checkbox v-if="ffqx4.length<5||editForm.czqxs.indexOf('4')!=-1"  label="4">风口决策</el-checkbox>
                    <el-checkbox v-else disabled label="4">风口决策</el-checkbox>
                ...
                qbkj:[
                  ...
                  {validator:async (rule, value, callback)=>{
                      if(!value){
                        ...
                        // if(ss.join(';')=='3;4;5'){
                        //     let resp=await api.existQbkjOfRightBottomCarousel({type:2})
                  ...
                  {validator:(rule, value, callback)=>{
                        ...
                        if(this.activeName==2){
                            this.datas.forEach(d=>{
                                if(d.link==value&&this.excludeVailLink!=value){
