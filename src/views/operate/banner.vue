		...
		<el-table :data="datas" highlight-current-row v-loading="listLoading" style="width: 100%;">
		  ...
		  <el-table-column label="操作" width="250">
		     <template scope="scope">
                ...
                <el-button size="small" type="info" v-if="scope.$index==0" >置顶</el-button>
                 <el-button type="danger" size="small"  @click="handletop(scope.$index,  scope.row.banner_id)" v-else >置顶</el-button>
...
export default {
    ...
    methods: {
         ...
         handleDel: function(row) {
                ...
                let result = await api.deleteBannercy(row.banner_id);
                if (result == 1) {
        ...
        //显示编辑界面
           ...
           handletop: function (index,id) {
               ...
               this.$confirm('确认置顶吗?', '提示', {
                        ...
                        let result=await api.setbannertopcy(id);
                        this.listLoading = false;
                        this.getlunbo();
                    }).catch(() => {
           ...
           handleJy: function (index,id) {
               this.$confirm('确认停用吗?', '提示', {
                        ...
                        let result=await api.setjybanner(id);
                        this.getlunbo();
                    }).catch(() => {
                    });
            },
            handleQy: function (index,id) {
               this.$confirm('确认启用吗?', '提示', {
                        ...
                        let result=await api.setqybanner(id);
                        this.getlunbo();
                    }).catch(() => {
                    });
            },
        async addSubmit() {
               this.$refs.editForm.validate(async (valid) => {
                    ...
                    this.$confirm('确认提交吗？', '提示', {}).then(async () => {
                        ...
                        await api.addbannercy(para)
                    }else{
                        let para = Object.assign({}, this.editForm);
                        await api.updatebannercy(para)
                    }
        ...
        async getlunbo() {
            ...
            let paras = {  };
            let list=await api.getbannercy(paras);
            this.datas=list;
         },
        //恢复编辑页面为默认
        clearEdit(){
            ...
            handleAvatarSuccess(res, file) {
              ...
              //  this.editForm.cover ='http://app2.zslxt.com/image/' + res.message;
               // this.editForm.cover ='http://ydgp.zslxt.com/image/' + res.message;
            },
