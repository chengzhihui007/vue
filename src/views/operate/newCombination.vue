		...
		<el-table :data="combination" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column label="操作" width="300">
				<template scope="scope">
					<el-button size="small" @click="handleEdit(scope.row,scope.row.name)" v-if="scope.row.del_flag==1">编辑板块</el-button>
					<el-button size="small" @click="handleEdit(scope.row,scope.row.name)"  v-else="" disabled>编辑板块</el-button>
		...
		<el-dialog :title=dialogTitle  :visible.sync="editFormVisible" :close-on-click-modal="false">
			<el-form :model="editForm" label-width="100px"  ref="editForm">
                <div>
					...
					<!-- <el-select  v-model="editForm.name"   :disabled="isClose==='1'" @change="findMenu(editForm,editForm.name)" >
                <el-option v-for="i in stars" :key="i.value" :label="i.label" :value="i.value"></el-option>
                </el-select> -->
				<el-input placeholder="五星"  :disabled="true"   style="width:300px;"></el-input>
				</el-form-item>
				</div>
				<div>
				<el-form-item label="功能选择：" prop="menus" >
<!-- 				<el-checkbox  v-model="item.status"   v-for="(item, k1) in sysMenu" :key="k1" :label="item.menus" @change="selectAllcheck(item.id,item.status)"></el-checkbox>
 --> 				<el-radio-group v-model="lh">
 					<el-radio   :disabled="lhClose==='1'"  v-for="(it, k) in sysMenu" :key="k"  :label="it.id" >{{it.menus}}</el-radio>
 				</el-radio-group> 
 				</el-form-item>
				</div>
...
<script>
		...
		methods: {
            ...
            handleDel: function (index, row) {
				...
				if(row.del_flag==1){// 1是启用  0 是禁用 
				    api.findNewCombinationByStar().then((data)=>{
                        if(data.data == 0){
                             this.$message({
				 ...
				 const confirmText = [this.editForm.name +"("+this.editForm.combination_name+")", '当前小组被禁用后，该小组下用户将不能查看相关信息，是否确认禁用?'] 
						  ...
						  let result=await api.deletNewCombination(this.editForm.id,this.editForm.del_flag);
						  this.$message({
                                message: '已禁用',
                                type: 'success'
                                });
            ...
            async getCombination(){
				...
				let result = await api.newCombinationInfoList(this.page,this.pageSize) 
				result = result.data;
			...
			async handleEdit(row,name) {
				...
				let result=await api.findStarByExperts(name);///star/findStarList 参数 star_name
				for (var i = 0; i < result.data.length; i++) {
					this.sysMenu.push(result.data[i])
					if(result.data[i].menus == row.block_name){
	...
	/* 		if(this.dxsysMenu.length == 1){//龙虎实战
            ...
            async handleAdd() {
				...
				//this.startName='五星';
				//this.id ="";
				let result=await api.findStarByExperts();//专家服务显示
				    for (var i = 0; i < result.data.length; i++) {
					this.sysMenu.push(result.data[i])
					}
			    this.lh=this.sysMenu[0].id //默认选中第一个
				this.editFormVisible = true;
			...
			/* 			this.dxsysMenu.forEach(element => {
						...
						let ceshi= sysMenu
						 api.findNewCombinationByNameAndBlack({datainfo:ceshi}).then((data) =>{
                             if(data.data !=null){
                                    this.msg = '该组合已存在，请勿重复添加'
                                }else{
							this.$confirm('确认提交吗？', '提示', {}).then( async () => {
							this.editLoading = true;
							let result=await api.addmenuByStarAndC({id:this.id,datainfo:ceshi});///star/addmenuBytar
