        ...
        <el-dialog  :title=dialogTitle  :visible.sync="dialogVisible" :close-on-click-modal="false" @close="handleClose">
            <el-form :model="editForm" label-width="100px" :rules="editFormRules" ref="editForm">
                ...
                <el-form-item label="套系权限：" prop="jurisdictionValue">
                    <el-select v-model="editForm.jurisdictionValue"  @change="selectjurisdiction" clearable placeholder="请选择套系权限">
    ...
    export default {
        ...
        var checkintroduce = (rule, value, callback) => {
            ...
            return {
                msg:'',
                optionsDJ:[
                    {jurisdiction:'免费',id:0, jurisdictionValue: 0},
                    {jurisdiction:'多头启动用户及以上',id:-2, jurisdictionValue: -2},
                    {jurisdiction:'多空决策及以上',id:3, jurisdictionValue: 3},
                    {jurisdiction:'风口决策及以上',id:4, jurisdictionValue: 4},
                    {jurisdiction:'主力决策',id:5, jurisdictionValue: 5}
                ],
                username:'',
                value: '',
                setVisible:false,
         ...
         /*        filters: {
                ...
                editFormRules: {
                    title: [
                        ...
                        { validator: checkname, message: "套系名称不能输入空格!", trigger: ['change', "blur"]}
                    ],
                    introduce:[
                        { required: true, message: "请填套系介绍", trigger: "blur" },
                        { min: 1, max: 200, message: "套系介绍不能超过200个汉字", trigger: "blur" },
                        { validator: checkintroduce, message: "套系介绍不能输入空格!", trigger: ['change', "blur"]}
                    ],
                     jurisdictionValue: [
        ...
        methods: {
            ...
            //获取列表
            async getList(){
                let result=await api.findindexClassManageList(this.page,this.pageSize,this.queryKeywords);
             ...
             async getListFind(){
                this.page = 1;
                this.pagesize = 1;
                let result=await api.findindexClassManageList(1,this.pageSize,this.queryKeywords);
                this.datas=result.data.result;
                this.total = result.data.total
                ...
                if(row.delete==0){
                  this.$confirm('若禁用该内容，则该套系不可添加课程，但不影响已添加内容展示，确定要禁用该分类吗？', '提示', {
                        type: 'warning'
                            ...
                            let jinyong = 1;
						  let result=await api.deleteIndexClassManage(this.editForm.id,jinyong);//禁用
						  this.$message({
                                message: '已禁用',
                                type: 'success'
                                });
                    ...
                    this.$confirm('若启用该内容，则该套系可继续添加该分类课程，是否确认启用？', '提示', {
                        ...
                        this.editForm.delete=0;
                         let jinyong = 0;
						let result=await api.deleteIndexClassManage(this.editForm.id,jinyong);
                        //this.listLoading = false;
						this.getList();
						 this.$message({
                            message: '已启用',
            ...
            async handleEdit (index, row) {
                ...
                this.oldJurisdiction = para.jurisdiction;
                this.currentTitle = this.editForm.title;
            },
            //显示新增界面
            handleAdd: function () {
                this.msg ='';
                ...
                this.$refs.editForm.validate(async (valid) => {
                    if (valid) {
                        if(this.editOrAdd==true){
                            this.msg ='';
                            let para = Object.assign({}, this.editForm);
                         let results= api.findIndexClassByTitle(para).then( async (data) => {
                                if(data.data !=null){
                                    this.msg = '该名称已存在';
                                     this.$message({
                                    ...
                                    api.zexistIndexClassManage(para.id).then(async (data) => {
                                        para.account = JSON.parse(sessionStorage.getItem('user')).name;
                                        this.$confirm('确认提交吗？', '提示', {}).then( async () => {
                                            this.editLoading = true;
                                            let result=await api.addIndexClassManage(para);///groups/addGroupsAndMenu
                                            if(result.data){
                                                this.$message({
                                                    message: '添加成功',
                                                    type: 'success'
                            ...
                            let para = Object.assign({}, this.editForm);
                            let results = api.findIndexClassByTitle(para).then( async (data) => {
                                if(data.data != null && data.data.title != this.currentTitle){
                                    this.msg = '该名称已存在';
                                    this.$message({
                                        message: '该名称已存在，请勿重复添加',
                                        type: 'error'
                                    ...
                                    api.zexistIndexClassManage(para.id).then(async (data) => {
                                        if (this.oldJurisdiction != para.jurisdiction && !data.state) {
                                           ...
                                           this.$confirm('确认提交吗？', '提示', {}).then( async () => {
                                               this.editLoading = true;
                                               let para = Object.assign({}, this.editForm);
                                               let result=await api.eidtIndexClassManage(para);
