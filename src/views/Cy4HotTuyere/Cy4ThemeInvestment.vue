    ...
    <section>
        ...
        <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
            <el-form :inline="true" :model="filters">
                ...
                <el-form-item>
                    <!-- <el-date-picker v-model="filters.create_time" type="date" placeholder="发布时间查询">
                    </el-date-picker> -->
        ...
        <el-table  ref="multipleTable" :data="datas"  highlight-current-row v-loading="listLoading" :fit="true" @selection-change="selsChange" style="width: 150%;">
            <el-table-column type="selection" width="55"></el-table-column>
             ...
             <el-table-column prop="id" label="ID"  min-width="80" >
                <template scope="scope">
                    <div class="deleted-text" v-if="scope.row.type == 1">  {{scope.row.id }}</div>
            ...
            <el-table-column prop="operation_user" label="最近操作账号" min-width="80">
                <template scope="scope">
                    <div class="deleted-text" v-if="scope.row.type == 1">{{ scope.row.operation_user }}</div>
                    <div v-else>{{ scope.row.operation_user }}</div>
                </template>
            </el-table-column>
            <el-table-column prop="updateTime" label="最近编辑时间" min-width="100" >
                <template scope="scope">
                    <div class="deleted-text" v-if="scope.row.type ==1">{{ scope.row.update_time }}</div>
                    <div v-else>{{ scope.row.update_time }}</div>
                </template>
            </el-table-column>
        ...
        <el-dialog :title=dialogTitle  :visible.sync="dialogVisible" :close-on-click-modal="false">
            <el-form  :model="editForm" :rules="rules" ref="editForm" label-width="100px" >
                ...
                <el-form-item label="板块名称" prop="plate_name">
                    ...
                    <div>
                        <el-select multiple  v-model="editForm.plate_name" filterable remote clearable placeholder="输入板块名称/代码搜索,最多添加不超过5个板块" v-on:clear="add_select_clear" :remote-method="select_remoteMethod"
    ...
    export default {
        ...
        fileInput.click();
                ...
                rules:{
                    ...
                    theme_pic:[   
                        {required: true, message: '请上传主题解读封面图', trigger: 'blur'},
                    ],
                    theme_reason: [   
                    {required: true, message: '请填写主题解读内容', trigger: 'blur'}
                ]
                },
                startTime: {
                    disabledDate: time => {
        ...
        methods: {
            ...
            history(){
                this.$router.push({path:'/cy4ThemeInvestmentHistory'})
            },
            add:function(){
                this.selectedList=[];
                this.editOrAdd = true;
