	...
	<section class="mtop">
			...
			<el-form :inline="true">
				...
				<el-form-item>
					...
					<el-table :data="starComeTables" highlight-current-row v-loading="loading"  @selection-change="selsChange"  style="width: 100%;">
                        ...
                        <template scope="scope">
                          <p>  {{scope.row.qx==-1?'全部可见':scope.row.qx==4?'风口决策用户可见':scope.row.qx==5?'主力决策用户可见':''}}</p>
                        </template>
						</el-table-column>
		...
		<el-dialog title="批量点赞"
			...
			<el-dialog title="编辑" :visible.sync="editclass"  :close-on-click-modal="false" width="50%" center>
				<el-form :model="editruleForm" :rules="editrules" ref="editruleForm" label-width="160px" >
					...
					<el-form-item label="老师" prop="teacher" >
						<el-select  v-model="editruleForm.teacher" @change="selectTeacher" clearable placeholder="--请选择老师--"  >
							<el-option
									v-for="list in teacherFrom"
									:key="list.id"
									:label="list.name"
					...
					<el-form-item label="老师执业资格信息" prop="teacDescription">
						<el-input v-model="editruleForm.teacDescription" readonly placeholder="选择老师后自动填写，若想修改老师执业信息请到功能管理-老师管理"></el-input>
					</el-form-item>
    ...
    export default {
        data() {
            ...
            return {
                ...
                listLoading: false,
                teacherFrom:{},
                qxList:[{id:-1,name:'全部可见'},{id:4,name:'风口决策用户可见'},{id:5,name:'主力决策用户可见'}],
				qxList1:[{id:-1,name:'全部可见'},{id:4,name:'风口决策用户可见'},{id:5,name:'主力决策用户可见'}],
                lableFrom:{},
                lableNameFrom:{},
                teacher:"",
                qx:'',
				...
				editrules: {
                  ...
                  {validator:(rule, value, callback)=>{
					...
					teacDescription: [
						{
							required: true, message: '请到功能管理-老师管理中填写该老师执业资格信息', trigger: 'change'
						}
					]
        ...
        methods: {
			selectTeacher(val) {
				...
				if (teac) {
					this.$set(
							this.editruleForm, 'teacDescription', teac.description
					);
				} else {
					this.$set(
							this.editruleForm, 'teacDescription', ''
					);
				}
				this.$forceUpdate();
			},
            ...
            selectLableNameList(flag){
				if (flag == 1) {
					...
					this.qxList = [{id:-1,name:'全部可见'},{id:4,name:'风口决策用户可见'},{id:5,name:'主力决策用户可见'}];
				}
			...
			*/
            ...
            handleEdit(index, row) {
				...
				this.selectTeacher(this.editruleForm.teacher);
            }
            //-------------------------
