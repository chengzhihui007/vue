    ...
    <section>
        ...
        <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
            <el-form :inline="true" :model="filters">
                <el-form-item>
                    <el-input v-model="filters.title" placeholder="请输入课题关键字"></el-input>
                </el-form-item>
                <el-form-item>
        ...
        <el-table  ref="multipleTable" :data="datas" highlight-current-row v-loading="listLoading"   @selection-change="selsChange"  style="width: 100%;">
            ...
            <el-table-column label="播放实数|总数" width="160">
                <template scope="scope">
                    <span v-bind:class="{active: (scope.row.viewR)}"  style="color: blue;">{{ scope.row.viewR }}</span>|<span>{{ scope.row.view }}</span>
                </template>
            </el-table-column>

            <el-table-column label="点赞实数|总数" width="160">
                <template scope="scope">
                    <span v-bind:class="{active: (scope.row.likeR)}" style="color: blue;">{{ scope.row.likeR }}</span>|<span>{{ scope.row.like }}</span>
                </template>
            </el-table-column>
            ...
            <el-table-column label="操作" min-width="150">
                <template scope="scope">
                    <template  v-if="scope.row.status == 1" >
                        ...
                        <el-button type="danger" size="small"   @click="handleDown(scope.$index, scope.row)">下线</el-button>
                       <!-- <el-button type="danger" size="small"   @click="handleDel(scope.$index, scope.row)">删除</el-button>-->
                    </template>
                    <template  v-else-if="scope.row.status == 2" >
                        <el-button type="warning" size="small"  disabled  @click="handleEdit(scope.$index, scope.row)">修改</el-button>
                        <el-button type="danger" size="small" disabled   @click="handleDown(scope.$index, scope.row)">已下线</el-button>
                        <!--<el-button type="danger" size="small" disabled=true   @click="handleDel(scope.$index, scope.row)">删除</el-button>-->
                    </template>
                </template>
        ...
        <el-dialog :title=dialogTitle  :visible.sync="FormVisible"  v-loading="editLoading" :close-on-click-modal="false" v-on:close="clearEdit" width="50%">
            <el-form :model="addForm" label-width="110px"  :rules="editFormRules" ref="addForm">
               ...
               <!-- <el-form-item label="文件地址：" prop="play_address">
                    <el-input v-model="addForm.play_address"  auto-complete="off"  placeholder="请输入地址" style="width:500px;"></el-input>
                </el-form-item>-->
                <el-form-item prop="default_date" label="发布时间">
                    <el-date-picker
              ...
              <!--  <el-form-item label="课程简介："  prop="content">
                ...
                <el-form-item label="音频文件" prop="play_address">
                    <el-upload  class="upload-demo" action="https://wangyi.zslxt.com/cloud/ondemand/uploads" accept=".mp3" :on-success="handleChangeAddAF" :on-remove="handleRemoveAddAF" :limit="1" :on-exceed="handleExceed" :file-list="playAddressFileList">
                        <el-button size="small" type="primary">点击上传音频</el-button>
                        <div style="float: right;margin-left: 15px;" class="el-upload__tip">点击上传音频</div>
                    </el-upload>
                </el-form-item>
                <el-form-item label="音频文件" prop="play_address" style="display: none" >
                    <el-input   v-model="addForm.play_address"></el-input>
                </el-form-item>
        ...
        <el-dialog :title="BatchViewDialogTitle" :visible.sync="BatchViewDialogVisible" >
            <el-form  :model="viewlikeFrom" ref="viewlikeFrom"  label-width="80px"  class="demo-ruleForm"  >
                ...
                <el-form-item>
                    <p>您确定为这些标题手动增加 <font size="5" color="red">{{viewlikeFrom.count}}</font>   个{{BatchlikeViewContent}}吗？</p>
                </el-form-item>
            </el-form>
    ...
    export default {
        ...
        methods: {
            ...
            async getList(){
               ...
               // var a = JSON.parse(admin);
                ...
                let para = {
                    ...
                    create_time:this.filters.createTime,
                ...
                let result=await api.ygcjfmList(para);
                for( var i=0;i<result.list.length;i++)
                {
                  var date = new Date(result.list[i].create_time);
                   ...
                   result.list[i].create_time=Y+M+D+h+m+s;
                    //result.list[i].create_time = util.formatDate.format(date,"yyyy-MM-dd hh:mm:ss");
                }
            ...
            handleRemoveAddAF(response, fileList) {
                this.addForm.play_address = ""
                this.addForm.audio_length = ""
            },
            handleChangeAddAF(response, file, fileList) {
                ...
                ws.onmessage = function (evt) {
                    ...
                    _this.addForm.audio_length = data.durationMsec
            ...
            async handleDel(index, row) {
                this.$confirm('确认删除该记录吗?', '提示', {
                    ...
                    var result= await api.ygcjfmdelete(row.fm_id);
            ...
            async handleDown(index, row) {
                this.$confirm('确认要下线该FM信息吗?', '提示', {
                    ...
                    var result = await api.ygcjfmupdate(this.addForm)
                    this.listLoading = false;
                    if(result.status=="1")
                    {
                        this.$message({
            ...
            handleEdit (index, row) {
                ...
                this.default_date = row.create_time;
            ...
            handleAdd: function () {
                ...
                this.default_date = util.formatDate.format(new Date(),"yyyy-MM-dd hh:mm:ss");
                this.playAddressFileList = [];
                this.teacherid="";
                this.teachername="";
                this.clearEdit();
            ...
            //提交
            async editSubmit() {
                console.warn(this.default_date)
            ...
            batchLikeView(type,title){
                ...
                if(this.BatchViewDialogTitle == '批量浏览'){
                    this.BatchlikeViewContent = "浏览";
                }
                if(this.BatchViewDialogTitle == '批量点赞'){
                    this.BatchlikeViewContent = "点赞";
                }
                    ...
                    if (valid) {
                        ...
                        for (var i = 0; i < this.sels.length; i++){
                            ...
                            var para = {
                                ...
                                fm_id:this.sels[i].fm_id
                            }
                            dataArray.push(para);
                        }
                        console.warn(dataArray)
                        var result = await api.ygcjfmbatchLikeView(dataArray);
                        if (result.status == "1") {
                            this.$message({
                                message: '注水成功',
                                type: 'success'
            ...
            setInfuse(type) {
                if (type =="like") {
                    this.sels.forEach(a => {
                        if ( Number(this.viewlikeFrom.count) + Number(a.like) > Number(a.view)) {

                            this.$message({
                                message: "超过阅读量已调整为阅读量最低！",
                                type: "warning"
                            });
                            a.zs_count = Number(a.view) - Number(a.like);
                            a.like = a.view;
                        } else {
