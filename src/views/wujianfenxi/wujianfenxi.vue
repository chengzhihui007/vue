<template>
 ...
 <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
        <el-form :inline="true" :model="filters">
            ...
            <el-form-item>
                <el-select  v-model="filters.name" clearable placeholder="请选择所属产品">
                    <el-option
...
<el-table :data="tableData" @selection-change="selsChange" highlight-current-row v-loading="listLoading" style="width: 100%;">
  ...
  <el-table-column prop="title" label="标题" min-width="120">
    ...
    <el-table-column prop="usertypeStr" label="所属产品"  :formatter="usertypeFormat"  min-width="120">
    </el-table-column>
...
<el-dialog v-if="editFormVisible" :title=dialogTitle :visible.sync="editFormVisible" :close-on-click-modal="false" v-on:close="clearEdit" width="50%">
    <el-form :model="editForm" label-width="110px" :rules="editFormRules" ref="editForm">
        <el-form-item label="所属产品：">
            <el-radio v-model="radio" label="1">财源四星产品</el-radio>
            <el-radio v-model="radio" label="2">财源五星产品</el-radio>
        </el-form-item>
...
<script>
    import {analyList,analysave,analyupdate,analydelect,analyupdateview} from "../../api/cyApi.js";
        ...
        methods: {
            ...
            async findlist(para) {
                para = {
                    ...
                    usertype:this.filters.name,
                    create_time:this.filters.datetime,
                };
                console.log(para)
                analyList(para).then(res => {
            ...
            handleCurrentChange(val) {
                ...
                var para = {
                    pageNo:this.page,
                    pageSize:this.pageSize,
                    orderBy:"a.noonanalysis_id DESC "
                };
                this.findlist(para);
            },
            alertMessage(msg) {
            ...
            handleEdit: function(index, row) {
                ...
                this.radio=this.editForm.usertype;
            },
            //显示新增界面
            handleAdd: function() {
                this.dialogTitle = "新增";
                    ...
                    if(this.InfuseType==0 && this.zhushuiid!=""){
                        this.$confirm('条数为'+this.zhushuiid+"的点赞量超过阅读数，是否取阅读数最低值？", '提示', {
                            ...
                            if (valid) {
                                ...
                                analyupdateview(this.sels).then(res => {
                                    this.$message({
                                        message: "注水成功",
                                        type: "success"
                                    });
...
//                    console.log(row)
//                    var paradel = {
//                        noonanalysis_id:row.noonanalysis_id
//                    };
//
                ...
                this.$refs.editForm.validate(async(valid) => {
                    if (valid) {
                        ...
                        this.$confirm('确认提交吗？', '提示', {}).then(async() => {
                            ...
                            this.editForm.usertype=this.radio;
                            ...
                            if (this.editOrAdd) {
                                ...
                                //更新
                                if(para.video_length==0){
                                    delete para.video_length;
                                }
                                delete para.create_time;
                                para.update_time=String( new Date().getTime());
                                analyupdate(para).then(res => {
                                    this.$message({
                                        message: "发布成功",
                                        type: "success"
                                    });
